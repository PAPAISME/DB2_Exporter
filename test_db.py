...
async def db_fixture(db_config):
    """DB."""
...
class TestInvalidResultCount:
    """Test for InvalidResultCount class."""
...
class TestCreateDBEngine:
    """Test for create db engine function."""
...
class TestQuery:
    ...
    def test_instantiate_parameters_not_matching(self):
        ...
        with pytest.raises(InvalidQueryParameters) as error:
            Query(
                "query",
                ["db1", "db2"],
                [
        ...
        assert (
            str(error.value)
            == "Parameters for query 'query' don't match those from SQL"
        )
    ...
    def test_timed(self, kwargs, is_periodic):
        """Test for the property for is_periodic in Query instance.

        is_periodic reports whether the query is run with a time schedule.
        ...
        assert query.is_periodic == is_periodic
...
class TestDataBase:
    ...
    async def test_connect(self, caplog, db):
        """The connection connects to the database."""
