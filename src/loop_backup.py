...
from toolrack.aio import PeriodicCall, TimedCall
...
class MetricsLastSeen:
    ...
    def __init__(self, expirations: dict[str, Union[int, None]]):
...
class QueryLoop:
    ...
    def __init__(
        ...
        self._last_seen = MetricsLastSeen(
            {
                name: metric.options.get("expiration")
    ...
    def _update_metric(
        ...
        if metric.type == "counter" and not metric.options.get("increment", True):
            method = "set"
